...
# https://github.com/yoshisatoh/Data_Pre-processing/blob/main/data_split/train_validation_and_test/random/data_split_train_valid_and_test__cs_random.py
#
#
########## Input Data File(s)
#
...
#
#python data_split_train_valid_and_test__cs_random.py df.csv y 0.8 0.1 None
#python data_split_train_valid_and_test__cs_random.py df.csv y 0.8 0.1 1
#
#Generally,
#python data_split_train_valid_and_test.py (dfname: data file name) (column name of target y to predict) (train_size) (valid_size) (random_state: 'None' for random splitting or integer to fix the splitting result)
#
...
train_size = float(sys.argv[3])    #0.8

valid_size = float(sys.argv[4])    #0.1
...
if (str(sys.argv[5]) == 'None'):
    random_state = None
elif (isinstance(int(sys.argv[5]), int)):
...
df = pd.read_csv(dfname, low_memory=False)
